...
/*
Name: Cole Groen
Class: COMP 3361 Section 1
Assignment: Assignment 2: Executing user programs
*/
...
void run(uint32_t LBA, uint32_t n_sectors) {
	...
	// use assembly, pushfl to push eflags into stack then use popl to pop stack into pcb
	...
	// fill data in the PCB for the user program (defined in the
    	...
    	//          a) memory base -> load base
    	//          b) memory limit -> bytes needed
    	//          c) stack segment (SS) -> 0x23
    	//          d) stack pointer (ESP): stack should begin from (end of process space - 4KB)
    	//          e) code segment (CS) -> 0x1B
...
__attribute__((fastcall)) void switch_to_user_process(PCB *p) {
    ...
    // TODO: set TSS.esp0 
	TSS.esp0 = user_program.memory_base + user_program.memory_limit;
	...
	gdt[3].base_16_23 = ((p->memory_base) & 0x00FF0000)>>16;
	gdt[3].access_byte = 0b11111010;
	gdt[3].limit_and_flag = 0b01000000 + (((p->memory_limit) & 0x000F0000)>>16);
	gdt[3].base_24_31 = ((p->memory_base) & 0xFF000000)>>24;

	gdt[4].limit_0_15 = (p->memory_limit) & 0xFFFF;
	gdt[4].base_0_15 = (p->memory_base) & 0xFFFF;
	gdt[4].base_16_23 = ((p->memory_base) & 0x00FF0000)>>16;
	gdt[4].access_byte = 0b11110010;
	gdt[4].limit_and_flag = 0b01000000 + (((p->memory_limit) & 0x000F0000)>>16);
	gdt[4].base_24_31 = ((p->memory_base) & 0xFF000000)>>24;
